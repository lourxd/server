...
import { binaryStatus } from '$srv/cloudflare/tunnels.js';
import { listUsers, activeSessionCount } from '$srv/auth.js';
import { recent } from '$srv/store/audit.js';

export async function load({ locals, request }) {
  const isAdmin = locals.user?.role === 'admin';
  const [config, github, cloudflare, binary, users, audit] = await Promise.all([
    publicSettings(),
    githubStatus(),
    cloudflareStatus(),
  ...
  return { config, github, cloudflare, binary, users, audit, sessions: activeSessionCount(), dataDir: DATA_DIR, isAdmin };
}
