<script>
  ...
  import CloudflareTunnels from '$lib/components/CloudflareTunnels.svelte';
  import CloudflareDns from '$lib/components/CloudflareDns.svelte';
  import NetworkIcon from '@lucide/svelte/icons/network';
  import Boxes from '@lucide/svelte/icons/boxes';

  let { data } = $props();

  let tab = $state('traffic');
...
<div class="flex flex-col gap-3.5 p-5 pt-3.5 md:p-6 md:pt-3.5">
  <Tabs.Root bind:value={tab} class="space-y-3.5">
    <Tabs.List>
      <Tabs.Trigger value="traffic" class="gap-1.5"><Activity class="size-3.5" /> Traffic</Tabs.Trigger>
      <Tabs.Trigger value="tunnels" class="gap-1.5">
        <Cloud class="size-3.5" /> Tunnels
        {#if data.tunnels.length}
          <span class="tabular font-mono text-[10px]">{data.tunnels.length}</span>
        {/if}
      </Tabs.Trigger>
      <Tabs.Trigger value="dns" class="gap-1.5"><Globe class="size-3.5" /> DNS</Tabs.Trigger>
    </Tabs.List>

    <Tabs.Content value="traffic" class="space-y-3.5">
    ...
    <Tabs.Content value="tunnels" class="space-y-3.5">
      <CloudflareTunnels {data} />
    </Tabs.Content>

    <Tabs.Content value="dns" class="space-y-3.5">
      <CloudflareDns {data} />
    </Tabs.Content>
  </Tabs.Root>
</div>
